...
namespace EditorHelpers
{
    ...
    constexpr ShortcutErrorType DoKeysOverlap(DWORD first, DWORD second);
    ...
    constexpr bool IsValidShortcut(Shortcut shortcut);

    // Function to check if the two shortcuts are equal or cover the same set of keys. Return value depends on type of overlap
    constexpr ShortcutErrorType DoShortcutsOverlap(const Shortcut& first, const Shortcut& second);
    ...
    constexpr ShortcutErrorType IsShortcutIllegal(Shortcut shortcut);
}